	...
	class SP_WQV_Framework {
		...
		public static function add_admin_enqueue_scripts() {
			...
			wp_localize_script(
				...
				array(
					...
					'i18n'          => array(
						'confirm'         => esc_html__( 'Are you sure?', 'woo-quick-view' ),
						'typing_text'     => esc_html__( 'Please enter %s or more characters', 'woo-quick-view' ),
						'searching_text'  => esc_html__( 'Searching...', 'woo-quick-view' ),
						'no_results_text' => esc_html__( 'No results found.', 'woo-quick-view' ),
					),
				)
			);
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field['_notice'] ) ) {
				...
				$field            = array();
				$field['content'] = esc_html__( 'Oops! Not allowed.', 'woo-quick-view' ) . ' <strong>(' . $field_type . ')</strong>';
				$field['type']    = 'notice';
				$field['style']   = 'danger';
			...
			if ( ! empty( $field_type ) ) {
				...
				if ( class_exists( $classname ) ) {
					...
					echo '<p>' . esc_html__( 'Field not found!', 'woo-quick-view' ) . '</p>';
				}
			} else {
				echo '<p>' . esc_html__( 'Field not found!', 'woo-quick-view' ) . '</p>';
			}
