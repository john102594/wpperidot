...
if ( ! class_exists( 'SP_WQV_Framework_Options' ) ) {
	...
	class SP_WQV_Framework_Options extends SP_WQV_Framework_Abstract {
		...
		public function ajax_save() {
			...
			if ( ! $result ) {
				wp_send_json_error( array( 'error' => esc_html__( 'Error while saving the changes.', 'woo-quick-view' ) ) );
			} else {
		...
		public function set_options( $ajax = false ) {
			...
			if ( wp_verify_nonce( $nonce, 'sp_wqv_options_nonce' ) ) {
				...
				if ( ! $ajax && ! empty( $response['sp_wqv_import_data'] ) ) {
					...
					$importing    = true;
					$this->notice = esc_html__( 'Settings successfully imported.', 'woo-quick-view' );
				...
				if ( ! empty( $transient['reset'] ) ) {
					...
					$this->notice = esc_html__( 'Default settings restored.', 'woo-quick-view' );
				...
				if ( empty( $this->notice ) ) {
					$this->notice = esc_html__( 'Settings saved.', 'woo-quick-view' );
				}
		...
		public function add_options_html() {
			...
			// echo ( $this->args['show_form_warning'] ) ? '<div class="sp_wqv-form-result sp_wqv-form-warning">' . esc_html__( 'You have unsaved changes, save your changes!', 'woo-quick-view' ) . '</div>' : '';
			...
			echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="sp_wqv-expand-all" title="' . esc_html__( 'show all settings', 'woo-quick-view' ) . '"><i class="fas fa-outdent"></i></div>' : '';

			echo ( $this->args['show_search'] ) ? '<div class="sp_wqv-search"><input type="text" name="sp_wqv-search" placeholder="' . esc_html__( 'Search...', 'woo-quick-view' ) . '" autocomplete="off" /></div>' : '';

			echo '<div class="sp_wqv-buttons">';
			echo '<input type="submit" name="' . esc_attr( $this->unique ) . '[_nonce][save]" class="button button-primary sp_wqv-top-save sp_wqv-save' . esc_attr( $ajax_class ) . '" value="' . esc_html__( 'Save Changes', 'woo-quick-view' ) . '" data-save="' . esc_html__( 'Saving...', 'woo-quick-view' ) . '">';
			echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="sp_wqv_transient[reset_section]" class="button button-secondary sp_wqv-reset-section sp_wqv-confirm" value="' . esc_html__( 'Reset Tab', 'woo-quick-view' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'woo-quick-view' ) . '">' : '';
			// echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="sp_wqv_transient[reset]" class="button sp_wqv-warning-primary sp_wqv-reset-all sp_wqv-confirm" value="' . ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'woo-quick-view' ) : esc_html__( 'Reset', 'woo-quick-view' ) ) . '" data-confirm="' . esc_html__( 'Are you sure you want to reset all settings to default values?', 'woo-quick-view' ) . '">' : '';
			echo '</div>';
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						SP_WQV_Framework::field( $field, $value, $this->unique, 'options' );
								...
								echo '<div class="sp_wqv-no-option">' . esc_html__( 'No data available.', 'woo-quick-view' ) . '</div>';
			...
			if ( ! empty( $this->args['show_footer'] ) ) {
				...
				echo '<div class="sp_wqv-buttons">';
				echo '<input type="submit" name="sp_wqv_transient[save]" class="button button-primary sp_wqv-save' . esc_attr( $ajax_class ) . '" value="' . esc_html__( 'Save', 'woo-quick-view' ) . '" data-save="' . esc_html__( 'Saving...', 'woo-quick-view' ) . '">';
				echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="sp_wqv_transient[reset_section]" class="button button-secondary sp_wqv-reset-section sp_wqv-confirm" value="' . esc_html__( 'Restore', 'woo-quick-view' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'woo-quick-view' ) . '">' : '';
				echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="sp_wqv_transient[reset]" class="button sp_wqv-warning-primary sp_wqv-reset-all sp_wqv-confirm" value="' . ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'woo-quick-view' ) : esc_html__( 'Reset', 'woo-quick-view' ) ) . '" data-confirm="' . esc_html__( 'Are you sure you want to reset all settings to default values?', 'woo-quick-view' ) . '">' : '';
				echo '</div>';
